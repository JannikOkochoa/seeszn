...
// ─── Quick Wins ───────────────────────────────────────────────────────────────
// Kuratierte, schnell umsetzbare AI-/SEO-Maßnahmen aus dem SEESZN-Audit. Jede
// Karte beantwortet die drei Fragen des Decision Layer – was ist passiert,
// warum, was empfehlen wir – und wird über den bestehenden Button direkt zur
// Maßnahme. Design, Kartenstruktur, Badge und Button bleiben unverändert; nur
// die angezeigten Inhalte sind fest hinterlegt (kein GSC-Bezug in dieser
// Section, deshalb keine Impressionen/Position/CTR-Werte).

import { Fragment } from "react";
import { useWorkspace } from "../workspace";

/** Eine Karte der Section. `recommendation` wird zeilenweise dargestellt. */
interface CuratedQuickWin {
  key: string;
  title: string;
  what: string;
  why: string;
  recommendation: string[];
  /** Vorbelegte Beobachtung für den „Maßnahme anlegen“-Drawer. */
  observation: string;
}

/**
 * Zeilenweise Ausgabe innerhalb eines bestehenden <dd>. Leere Zeilen erzeugen
 * einen ruhigen Absatzabstand über die vorhandene Zeilenhöhe – ohne neue
 * Styles, Container oder Typografie.
 */
...
const QUICK_WINS: readonly CuratedQuickWin[] = [
  {
    key: "qw-entity-block",
    ...
    observation:
      "Zentralen „Was ist Klühspies?“-Block ergänzen: Eine kurze, eindeutig zitierbare Entity-Definition von Klühspies fehlt, damit AI-Systeme das Unternehmen korrekt zuordnen und zitieren können.",
  },
  {
    key: "qw-homepage-faq",
    ...
    observation:
      "Homepage-FAQ auf konkrete AI-Fragen erweitern: Fünf Klühspies-spezifische Frage-Antwort-Paare (Anzahlung, Einzelzahlungen, Portal, Erreichbarkeit, Erfahrung) vor den bestehenden allgemeinen Fragen ergänzen.",
  },
  {
    key: "qw-zahlen-fakten",
    ...
    observation:
      "Seite „Klühspies in Zahlen & Fakten“ erstellen: Zitierfähige Kennzahlen zentral bündeln und intern verlinken. Die Angebotszahl (148) dynamisch halten und die Anzahlungsangabe fachlich prüfen (keine unbestätigten 80 %).",
  },
  {
    key: "qw-ratgeber-hub",
    ...
    observation:
      "Planungsratgeber und neuen Hubartikel zusammenführen: Den neuen Hubartikel auf der bestehenden URL /ratgeber/organisation-einer-klassenfahrt/planung-der-klassenfahrt/ aktualisieren statt eine zweite konkurrierende URL aufzubauen; sonst 301 auf die Hauptquelle.",
  },
  {
    key: "qw-oai-searchbot",
    ...
    observation:
      "OAI-SearchBot und technischen Bot-Zugriff prüfen: robots.txt, CDN/Firewall, Bot-Schutz und Server-Logs auf Erreichbarkeit für OAI-SearchBot prüfen. Zugriff ist aktuell nicht verifiziert, nicht als blockiert behaupten; GPTBot separat behandeln.",
  },
];

export default function QuickWinsPanel() {
  const { canWrite, openCreate } = useWorkspace();
        ...
        {QUICK_WINS.map((w) => (
          <li key={w.key} className="kw-int-card">
            <div className="kw-int-card-head">
              <span className="kw-int-subject">{w.title}</span>
